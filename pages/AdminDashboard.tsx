...
const AdminDashboard: React.FC = () => {
  ...
  const SidebarItem = ({ tab, icon: Icon, label }: { tab: Tab, icon: React.ElementType, label: string }) => (
    <button
      ...
      className={`w-full flex items-center space-x-3 px-6 py-3.5 transition-colors ${
        activeTab === tab 
          ? 'bg-wood-50 text-wood-700 border-r-4 border-wood-600' 
          : 'text-concrete-500 hover:bg-concrete-50 hover:text-concrete-800'
      }`}
    >
      <Icon size={18} />
  ...
  return (
    <div className="min-h-screen bg-concrete-100 pt-20 flex">
      ...
      <aside className="w-64 bg-white border-r border-concrete-200 fixed h-full z-10 hidden md:block overflow-y-auto pb-20">
        ...
        <nav className="mt-2">
          <div className="px-6 pt-6 pb-2 text-xs font-bold text-concrete-400 uppercase tracking-wider">概览</div>
          <SidebarItem tab="overview" icon={LayoutDashboard} label="系统概览" />
          
          <div className="px-6 pt-6 pb-2 text-xs font-bold text-concrete-400 uppercase tracking-wider">首页模块</div>
          ...
          <div className="px-6 pt-6 pb-2 text-xs font-bold text-concrete-400 uppercase tracking-wider">其他模块</div>
...
const SettingsTab: React.FC = () => {
  ...
  return (
    <div className="max-w-4xl">
      ...
      <div className="bg-concrete-50 border border-concrete-200 rounded-xl p-8">
        ...
        <form onSubmit={handleSave} className="space-y-6">
          ...
          <div className="pt-4 flex justify-end">
             <button 
               ...
               className={`flex items-center px-6 py-3 rounded-lg text-white font-bold transition-all shadow-md ${
                 saveStatus === 'saved' ? 'bg-green-600' : 'bg-wood-600 hover:bg-wood-700'
               }`}
             >
            ...
            <Modal isOpen={!!editingItem} onClose={() => setEditingItem(null)} title="编辑亮点应用">
                {editingItem && (
                    <form onSubmit={handleSave} className="space-y-6">
                        ...
                        <div className="space-y-4">
                            <input type="text" placeholder="标题" value={editingItem.title} onChange={e => setEditingItem({...editingItem, title: e.target.value})} className="w-full p-2 border rounded font-bold" required />
                            <textarea placeholder="摘要 (简短描述)" value={editingItem.summary} onChange={e => setEditingItem({...editingItem, summary: e.target.value})} className="w-full p-2 border rounded" rows={2} required />
                            <textarea placeholder="详细描述 (支持 Markdown 风格)" value={editingItem.fullDescription} onChange={e => setEditingItem({...editingItem, fullDescription: e.target.value})} className="w-full p-2 border rounded" rows={5} required />
                            
                            <div className="grid grid-cols-2 gap-4">
                               <ImageUploader label="缩略图" value={editingItem.thumbnail} onChange={v => setEditingItem({...editingItem, thumbnail: v})} className="h-40" />
                               <div>
                                  <label className="block text-sm font-bold mb-2">视频演示 URL</label>
                                  <input type="text" value={editingItem.videoUrl || ''} onChange={e => setEditingItem({...editingItem, videoUrl: e.target.value})} className="w-full p-2 border rounded" placeholder="http://..." />
                                  <div className="mt-4 p-3 bg-gray-50 rounded text-xs text-gray-500">
                                      提示: 图片集和附件管理请在创建后或使用更高级编辑器完善 (当前简化版)
                                  </div>
                               </div>
                            </div>
                        </div>
